...
class Catalog:
    ...
    def upload_tracks_to_users(self, t_to_u_redis, recommendations_path="RECOMMENDATIONS_FILE_PATH"):
        recommendations_file_path = self.app.config[recommendations_path]

        self.app.logger.info(
            f"Uploading tracks to users to redis from {recommendations_file_path}"
        )

        j = 0
        all_tracks = set()
        with open(recommendations_file_path) as rf:
            users_to_tracks = pd.DataFrame(columns=['user', 'track'])
            for line in rf:
                recommendations = json.loads(line)
                for track in recommendations["tracks"]:
                    all_tracks.add(track)
                    users_to_tracks.append(
                        {
        ...
        for track in all_tracks:
            j += 1
            t_to_u_redis.set(track, self.to_bytes(users_to_tracks[track == users_to_tracks['track']]['user']))

        self.app.logger.info(f"Uploaded tracks to users for {j} tracks")
