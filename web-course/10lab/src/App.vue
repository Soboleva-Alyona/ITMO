<template>
    <div id="app">
        ...
        <Middle :posts="posts" :users="users" :comments="comments" :post="post"/>
        <Footer :usersCnt="Object.keys(this.users).length" :postsCnt="Object.keys(this.posts).length"/>
    </div>
</template>
    ...
    beforeCreate() {
      ...
      this.$root.$on("onRegister", (login, name) => {
        if (!(/^[a-z]{3,16}$/.test(login))) {
          this.$root.$emit("onRegisterValidationError", "Login should contain exactly 3-16 lowercase latin symbols");
          return;
        }
        if (name.length < 1 || name.length > 32) {
          this.$root.$emit("onRegisterValidationError", "Name should contain 1-32 symbols");
          return;
        }
        let foundUsers = Object.values(this.users).filter(u => u.login === login);
        if (foundUsers.length) {
          this.$root.$emit("onRegisterValidationError", "Required unique login");
          return;
        }
