...
public class FrontServlet extends HttpServlet {
    ...
    private void process(Route route, HttpServletRequest request, HttpServletResponse response)
            ...
            for (Method m : clazz.getDeclaredMethods()) {
                if (m.getName().equals(route.getAction()) && m.getReturnType() == void.class) {
                    ...
                    for (Class<?> parameterType : m.getParameterTypes()) {
                        if (parameterType != Map.class && parameterType != HttpServletRequest.class) {
        ...
        try {
            ...
            for (int i = 0; i < parametersTypes.length; i++) {
                if (parametersTypes[i] == Map.class) {
                    parameters[i] = view;
                }
                if (parametersTypes[i] == String.class) {
        ...
        Template template = newTemplate(pageClass, getLang(request, session));
    ...
    private String getLang(HttpServletRequest request, HttpSession session) {
    ...
    private boolean isValidLang(String lang) {
        return lang != null && lang.length() == 2
                && Character.isLetter(lang.charAt(0)) && Character.isLowerCase(lang.charAt(0))
                && Character.isLetter(lang.charAt(1)) && Character.isLowerCase(lang.charAt(1));
    }
    ...
    private Template newTemplate(Class<?> pageClass, String lang) throws ServletException {
        ...
        if (isValidLang(lang) && !lang.equals("en")) {
            try {
